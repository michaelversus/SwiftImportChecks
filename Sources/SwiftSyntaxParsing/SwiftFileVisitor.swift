...
/// Parses a swift file into a tree of nodes we can analyze.
final class FileVisitor: SyntaxVisitor {
    ...
    override func visit(_ node: FunctionDeclSyntax) -> SyntaxVisitorContinueKind {
        var throwingStatus = SwiftFunction.ThrowingStatus.unknown
        var isStatic = false
        var returnType = ""
        ...
        if let throwsKeyword = node.signature.throwsOrRethrowsKeyword {
            if let throwsOrRethrows = SwiftFunction.ThrowingStatus(rawValue: throwsKeyword.text) {
    ...
    func create(_ type: SwiftType.ObjectType, from node: CommonSyntax) {
        let nodeBody = "\(node)"
